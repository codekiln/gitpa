...
def validate_directories():
    directories = [
        RECORDINGS_ROOT_FOLDER.expanduser(),
        LOGSEQ_FOLDER,
        LOGSEQ_ASSETS_FOLDER,
    ]
...
def construct_episode_recording_dir_name(episode_date):
    ...
    """
    episode_recording_dir_name = (
        f"GitP.{episode_date.strftime(SHORT_DATE_FORMAT)} Project"
    )
...
def construct_path_to_episode_recording_dir(episode_date):
    ...
    return episode_recording_dir.expanduser()
...
def construct_path_to_episode_publishing_dir(episode_date, args):
    ...
    if args.reference:
        ...
        expected_ref_ep_dir = (
            REPO_ROOT
            / "gitp-acolyte/gitp_acolyte/ceremonial/spells/episode_reference/ref_ep_dir"
        )
        assert (
            ref_ep_dir == expected_ref_ep_dir
        ), f"Reference episode directory is not as expected: \nactual:   '{ref_ep_dir}' \nexpected: '{expected_ref_ep_dir}'"
...
def ensure_episode_publishing_dir(episode_date, args):
    ...
    """
    path_to_episode_publishing_dir = construct_path_to_episode_publishing_dir(
        episode_date, args
    )
    if not path_to_episode_publishing_dir.exists():
        path_to_episode_publishing_dir.mkdir(parents=True, exist_ok=True)
        logger.debug(
            f"Episode publishing directory created: {path_to_episode_publishing_dir}"
        )
        return True
    logger.debug(
        f"Episode publishing directory already exists: {path_to_episode_publishing_dir}"
    )
    return False
...
def ensure_episode_yaml(episode_date, args):
    ...
    if episode_yaml_path.exists() and args.recreate:
        ...
        logger.info(
            f"--recreate: Removed existing episode.yml file: {episode_yaml_path}"
        )
    ...
    if not episode_yaml_path.exists():
        ...
        if "episode_date" not in episode_data or episode_data[
            "episode_date"
        ] != episode_date.strftime(DATE_FORMAT):
            episode_data["episode_date"] = episode_date.strftime(DATE_FORMAT)
            with episode_yaml_path.open("w") as file:
                yaml.dump(episode_data, file)
            return True
...
def recording_dir_exists(episode_date, args) -> tuple[Path, bool]:
    ...
    logger.debug(
        f"Path to episode recording directory: '{path_to_episode_recording_dir}'"
    )
...
def episode_publishing_dir_exists(episode_date, args) -> tuple[Path, bool]:
    ...
    logger.debug(
        f"Path to episode publishing directory: {path_to_episode_publishing_dir}"
    )
    if not path_to_episode_publishing_dir.exists():
        if args.reference:
            logger.info("Reference episode directory does not exist. ")
        else:
            logger.debug(
                f"Episode publishing directory does not exist: {path_to_episode_publishing_dir}"
            )
...
def ensure_episode_dir_and_yaml_exists(episode_date, args) -> tuple[Path, Path]:
    ...
    if not ep_pub_dir_exists:
        episode_publishing_dir_created = ensure_episode_publishing_dir(
            episode_date, args
        )
